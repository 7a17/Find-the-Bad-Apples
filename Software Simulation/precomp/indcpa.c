...
void enc(unsigned char * c, 
         ...
         ct_info ctinf, int k, int select, int task_list[256])
{
  polyvec Pb = { .vec[0] = {0} };
  poly v = { .coeffs = {0} };
  ...
  for(int j=0;j<4;j++)
  {
    int sel = task_list[k*4+j];
    if(sel == 0)
    {
      if(j==0)
        Pb.vec[select].coeffs[0] = (int)(KYBER_Q/1024.0*ctinf.c1 + 0.5);
      if(j==1)
        Pb.vec[select].coeffs[0] = (int)(KYBER_Q/1024.0*ctinf.c2 + 0.5);
      if(j==2)
        Pb.vec[select].coeffs[0] = (int)(KYBER_Q/1024.0*ctinf.c3 + 0.5);
      if(j==3)
        Pb.vec[select].coeffs[0] = (int)(KYBER_Q/1024.0*ctinf.c4 + 0.5);
    }
    else
    {
      if(j==0)
        Pb.vec[select].coeffs[256-sel] = (int)(KYBER_Q/1024.0*(1024-ctinf.c1) + 0.5);
      if(j==1)
        Pb.vec[select].coeffs[256-sel] = (int)(KYBER_Q/1024.0*(1024-ctinf.c2) + 0.5);
      if(j==2)
        Pb.vec[select].coeffs[256-sel] = (int)(KYBER_Q/1024.0*(1024-ctinf.c3) + 0.5);
      if(j==3)
        Pb.vec[select].coeffs[256-sel] = (int)(KYBER_Q/1024.0*(1024-ctinf.c4) + 0.5);
    }
  }

  polyvec_reduce(&Pb);            // compress(Pb) = c1
  v.coeffs[0] = ctinf.v;                // c2[0] = h

  pack_ciphertext_Attack(c, &Pb, &v);    // pack all c
}

void enc_rec(unsigned char * c, 
         const unsigned char * m, 
