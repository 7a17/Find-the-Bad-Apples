...
{
    int ret=0;
	for(int i=0;i<VOTE_RECOLL_NUM;i++)
	{
		ret+=SCA_oracle(ct,sk,msg_A);
	}
    return (ret>(VOTE_RECOLL_NUM/2));
}
...
static int kyber_Attack(int r,int save[6]) {
    ...
    save[2] = query_check;
	/* vote the recollection especially for accu=0.990 */
    int query_reclt = 0;
